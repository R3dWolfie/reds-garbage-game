...
Shows splash, checks for updates, stages + applies via helper script.
"""
...
from updater.updater import (
    check_for_update, download_update, stage_update,
    apply_staged_update_and_restart, check_pending_update
)


def run_launcher():
    """Run the update checker using the existing pygame display."""
    # Check if we just applied an update
    if check_pending_update():
        print(f"[Launcher] Running newly updated version v{VERSION}")
    ...
    update_info = None
    error_message = ""
    state = "checking"  # checking, found, downloading, staging, restarting, no_update, error
    ...
    def download_and_stage_thread():
        ...
        try:
            ...
            # Step 2: Stage (extract to _update_staging/)
            state = "staging"
            success = stage_update(path)
            if not success:
                state = "error"
    ...
    while running:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_RETURN:
                    if state in ("no_update", "error"):
                        ...
                        state = "downloading"
                        progress = 0.0
                        dl_thread = threading.Thread(target=download_and_stage_thread, daemon=True)
                        dl_thread.start()
        ...
        if state == "checking":
            ...
            screen.blit(txt, (cx - txt.get_width() // 2, mid_y))
            auto_launch_timer -= 1
            secs = max(0, auto_launch_timer // 60)
            hint = font_small.render(f"Launching in {secs}s... (ENTER to skip)", True, (150, 150, 150))
            ...
            cl = update_info.get("changelog", "")
            if cl:
                ct = font_small.render(cl[:80], True, (180, 180, 180))
                screen.blit(ct, (cx - ct.get_width() // 2, mid_y + 15))
            ...
            txt = font_med.render("Preparing update...", True, (255, 200, 0))
            screen.blit(txt, (cx - txt.get_width() // 2, mid_y))
