...
def make_neon_sprite(base_image, glow_color, glow_size=4):
    """Wrap a sprite image with a neon glow outline.
    Returns a new Surface with the glow baked in."""
    w, h = base_image.get_size()
    new_w = w + glow_size * 2
    new_h = h + glow_size * 2
    result = pygame.Surface((new_w, new_h), pygame.SRCALPHA)

    # Draw layered glow rectangles
    for i in range(glow_size, 0, -1):
        alpha = int(40 * (i / glow_size))
        glow_rect = pygame.Rect(glow_size - i, glow_size - i, w + i * 2, h + i * 2)
        pygame.draw.rect(result, (*glow_color[:3], alpha), glow_rect, max(1, i))

    # Bright border
    pygame.draw.rect(result, (*glow_color[:3], 120), (glow_size - 1, glow_size - 1, w + 2, h + 2), 1)

    # Blit original sprite on top
    result.blit(base_image, (glow_size, glow_size))
    return result
