...
class PlayerTank(PlayerBase):
    ...
    def draw_ram_aura(self, surf):
        """Draw a neon aura showing the ram is active."""
        if self.collision_cooldown <= 0:
            aura_surf = pygame.Surface((self.rect.width + 20, self.rect.height + 20), pygame.SRCALPHA)
            pygame.draw.ellipse(aura_surf, (0, 150, 255, 35),
                                aura_surf.get_rect())
            pygame.draw.ellipse(aura_surf, (0, 200, 255, 60),
                                aura_surf.get_rect(), 2)
